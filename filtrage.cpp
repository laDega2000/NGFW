    ...
    if (s.isPacketInSession(pac.src_ip,pac.dst_ip,pac.src_port, pac.dst_port)) {
       ...
       base bb;
        ...
        for (Rule r : p->get("dega",bb).rules) {
            if (((pac.src_zone.type==r.src_zo)&&(pac.dst_zone.type==r.dst_zo))||(pac.src_ip == r.srcIP || pac.src_ip=="ANY") && (pac.dst_ip == r.destIP || pac.dst_ip=="ANY") && pac.src_port == r.srcPort && pac.dst_port == r.destPort && pac.users.name== r.us.name) {
                if (r.permit) {
