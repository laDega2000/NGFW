...
bool filtrage::execute( char *packet, int packet_len, Policy& p, tabses& s,tabdomain &tab) {
    ...
    if (s.isPacketInSession(src_ip, dst_ip, src_port, dst_port)) {
        ...
        for (Rule r : p.rules) {
            if ((src_ip == r.srcIP || src_ip=="ANY") && (dst_ip == r.destIP || dst_ip=="ANY") && src_port == r.srcPort && dst_port == r.destPort) {
                if (r.permit) {
