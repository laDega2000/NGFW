...
//const std::vector<std::string>& params
  ...
  //while(true){
    bi=i_conf->get_all(bi);
      for(interfaces i :bi.tab_interfaces ){
         ...
         while(x<50){
            ...
            if(var==true)
            {
             ...
             ob=new routage_stat;
                                                ...
                                                for(interfaces in : bi.tab_interfaces)
                                                {
                                                  if(in.ip_adr==pac.next_hop){i.output=in.output;}
                                                }
              /*ob=new nat;
              ...
              i.send(pac);
                  ...
                  if(std::regex_match(pac.dst_ip,regx)==true && (std::regex_match(pac.src_ip,regx)==false)){
                    ...
                    cont.set_strategy(ob);
                      ...
                      cont.set_strategy(ob);
                      pac=cont.execute_strategy(pac,r_conf,n_conf);
                      cout<<endl<<"aniiii natiiiiitttt"<<endl;
                      i.send(pac);
                                      ...
                                      if(std::regex_match(pac.dst_ip,regx)==false && (std::regex_match(pac.src_ip,regx))==true){
                                            ...
                                            cont.set_strategy(ob);
                                               ...
                                               packet pook;
                                                pook=cont.execute_strategy(pac, r_conf,n_conf);
                                                
                                                for(interfaces in : bi.tab_interfaces)
                                                {
                                                  if(in.ip_adr==pook.next_hop){i.output=in.output;}
                                                }
                                              ob=new nat;
                                              cont.set_strategy(ob);
                                              pac=cont.execute_strategy(pac,r_conf,n_conf);
                                         ...
                                         if(std::regex_match(pac.dst_ip,regx)==false && (std::regex_match(pac.src_ip,regx)==false)){
                                              ...
                                              cont.set_strategy(ob);
                                                ...
                                                packet pok;
                                                pok=cont.execute_strategy(pac, r_conf,n_conf);
                                                
                                                for(interfaces in : bi.tab_interfaces)
                                                {
                                                  if(in.ip_adr==pok.next_hop)
                                                  {
                                                    i.output=in.output;
                                                  }
                                                }
