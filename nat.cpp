...
int nat::get_new_public_port() {
 ...
 srand(static_cast<unsigned int>(time(0)));
   ...
   /* auto it = std::find_if(nat_translation.begin(), nat_translation.end(),
    ...
    for(Translation tito : nat_translation)
    {
        if(tito.translated_ip==packet.dst_ip && tito.translated_port==packet.dst_port){
                packet.dst_ip = tito.original_ip;
                packet.dst_port = tito.original_port;
        }
    return packet;
    }
